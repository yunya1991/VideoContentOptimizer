...
- 🎴 **询问/执行双模式** - 单视频走询问模式（Step 1 参数确认），批量走执行模式（默认值直接跑）
- 💡 **推荐卡片 + 参数表单** - AI 生成 3 张个性化优化方向卡片，或手动精细配置参数
- 🧬 **自主进化引擎** - 每次选择沉淀为进化材料，Soul 用户画像越用越懂你
- 🎬 **批量处理** - 支持批量文案与视频分析优化
- 📱 **平台适配** - 抖音/小红书/微信格式自适应
- 🔄 **流程闭环** - 从分析→优化→重生成→发布
...
1. **上传视频** - 在 Web UI 上传视频文件（支持 mp4, mov, avi, mkv，≤500 MB）
2. **自动分析** - 系统自动提取元数据、转录音频（Whisper）、识别意图
3. **Step 1 参数确认** - 选择优化方向：推荐卡片（AI 生成，多消耗约 500-1000 tokens）或参数表单（手动配置，无额外消耗）
4. **执行优化** - 生成优化文案、多个标题版本
5. **对比选择** - 对比原版和优化版，选择最佳方案
6. **发布分享** - 导出或发布到各平台（TikTok/YouTube 等）

> 💡 **批量处理**：切换到"执行模式"，粘贴多段文案，一次跑完，跳过 Step 1。
...
- [x] 标题生成 (LLM)
- [x] 平台适配建议
- [x] 询问/执行双模式（Step 1 参数确认）
- [x] 推荐卡片 + 参数表单
- [x] 自主进化引擎 + Soul 用户画像
- [x] TTS 配音（6 引擎）+ 字幕烧录
- [x] 跨平台发布（upload-post.com）
- [x] 批量处理
- [ ] A/B 测试对比 UI
- [ ] 数据统计分析
- [ ] 移动端适配
